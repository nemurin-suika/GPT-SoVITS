                                ...
                                if (installpath[:len(python_dir)].upper() == python_dir.upper()):
                                    ...
                                    if os.path.exists(os.path.join('TEMP', 'ko', 'ko_dict')):
                                        ...
                                        def _rm_readonly(func, path, _):
                                            ...
                                            func(path)
                                        shutil.rmtree(os.path.join('TEMP', 'ko', 'ko_dict'), onerror=_rm_readonly)
...
from text.en_to_ko import english_to_korean
...
def g2p(text):
    # 알파벳을 한 글자씩 읽지(latin_to_hangul) 않고 영어 발음에 가까운 한글로 변환
    text = english_to_korean(text)
    text = _g2p(text)
